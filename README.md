...
- LLM 없이 규칙 기반 품질 게이트를 적용해 회사명, 제목, 요약, 인사이트, 추천 액션, 중복 노출을 정리합니다.
- 카드 제목은 원문 기사 제목을 그대로 쓰지 않고 `주요 타이틀 - 서브 타이틀` 형식의 인텔리전스 제목으로 재생성합니다.
- 서브 타이틀은 실제 이벤트의 직접 영향만 사용합니다. 예를 들어 파업은 채용 운영이 아니라 노사 리스크, 조직 안정성, 핵심 인력 이탈 가능성과 연결합니다.
...
네이버 API Secret을 추가하면 국내 기사 수집 품질과 최신성, 네이버 뉴스 링크 확보율이 좋아집니다.
최종 노출 수보다 넓은 후보 풀을 먼저 수집한 뒤 품질 게이트와 중복 제거를 거쳐 대시보드에 반영합니다.
